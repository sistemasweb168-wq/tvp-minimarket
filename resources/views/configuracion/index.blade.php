...
<div x-data="{ tab: 'empresa' }" class="grid grid-cols-1 lg:grid-cols-4 gap-5">
    <div class="lg:col-span-1 bg-white rounded-2xl shadow-md p-3 h-fit">
    ...
    <div class="lg:col-span-3">
        <!-- Empresa -->
        <div x-show="tab=='empresa'" class="bg-white rounded-2xl shadow-md p-6">
            ...
            <form method="POST" action="{{ route('configuracion.empresa') }}" enctype="multipart/form-data">
                @csrf
                    ...
                    <div class="md:col-span-2"><label class="block text-sm font-semibold mb-1">Razón Social *</label><input type="text" name="razon_social" value="{{ $empresa->razon_social }}" required class="w-full px-3 py-2.5 border border-slate-300 rounded-lg"></div>
                    <div><label class="block text-sm font-semibold mb-1">Nombre Comercial</label><input type="text" name="nombre_comercial" value="{{ $empresa->nombre_comercial }}" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg"></div>
                    <div><label class="block text-sm font-semibold mb-1">RUC / NIT</label><input type="text" name="ruc_nit" value="{{ $empresa->ruc_nit }}" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg"></div>
                    <div class="md:col-span-2"><label class="block text-sm font-semibold mb-1">Dirección</label><input type="text" name="direccion" value="{{ $empresa->direccion }}" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg"></div>
                    <div><label class="block text-sm font-semibold mb-1">Ciudad</label><input type="text" name="ciudad" value="{{ $empresa->ciudad }}" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg"></div>
                    <div><label class="block text-sm font-semibold mb-1">Teléfono</label><input type="text" name="telefono" value="{{ $empresa->telefono }}" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg"></div>
                    <div><label class="block text-sm font-semibold mb-1">Email</label><input type="email" name="email" value="{{ $empresa->email }}" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg"></div>
                    <div><label class="block text-sm font-semibold mb-1">Sitio Web</label><input type="text" name="sitio_web" value="{{ $empresa->sitio_web }}" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg"></div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="moneda" value="{{ $empresa->moneda ?? 'S/' }}" maxlength="10" required class="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-2xl font-bold text-center">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold mb-1">Código Moneda *</label>
                        <select name="codigo_moneda" required class="w-full px-3 py-2.5 border border-slate-300 rounded-lg">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="impuesto" value="{{ $empresa->impuesto ?? 18 }}" required class="w-full px-3 py-2.5 border border-slate-300 rounded-lg">
                    </div>
                </div>
                ...
                <div class="space-y-3">
                    <div><label class="block text-sm font-semibold mb-1">Mensaje en Ticket</label><input type="text" name="mensaje_ticket" value="{{ $empresa->mensaje_ticket ?? '¡Gracias por su compra!' }}" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg"></div>
                    <div><label class="block text-sm font-semibold mb-1">Términos y Condiciones</label><textarea name="terminos_condiciones" rows="3" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg">{{ $empresa->terminos_condiciones }}</textarea></div>
                </div>
        ...
        <!-- General -->
        <div x-show="tab=='general'" x-cloak class="bg-white rounded-2xl shadow-md p-6">
            ...
            <form method="POST" action="{{ route('configuracion.general') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="config[puntos_por_moneda]" value="{{ $configs['puntos_por_moneda'] ?? '0.1' }}" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg">
                    <p class="text-xs text-slate-500 mt-1">Ej: 0.1 = el cliente recibe 1 punto por cada 10 unidades gastadas</p>
                </div>
                <div>
                    <label class="block text-sm font-semibold mb-1">Días de aviso por vencimiento</label>
                    <input type="number" name="config[dias_aviso_vencimiento]" value="{{ $configs['dias_aviso_vencimiento'] ?? '30' }}" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg">
                </div>
                <div>
                    <label class="block text-sm font-semibold mb-1">Stock mínimo predeterminado</label>
                    <input type="number" name="config[stock_minimo_default]" value="{{ $configs['stock_minimo_default'] ?? '5' }}" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg">
                </div>
        ...
        <!-- Facturación -->
        <div x-show="tab=='facturacion'" x-cloak class="bg-white rounded-2xl shadow-md p-6">
            <h2 class="text-xl font-bold mb-5"><i class="fas fa-file-invoice mr-2 text-emerald-500"></i>Facturación</h2>
            <form method="POST" action="{{ route('configuracion.general') }}" class="space-y-4">
                @csrf
                <div class="grid grid-cols-2 gap-4">
                    <div><label class="block text-sm font-semibold mb-1">Serie Ticket</label><input type="text" name="config[serie_ticket]" value="{{ $configs['serie_ticket'] ?? 'T001' }}" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg"></div>
                    <div><label class="block text-sm font-semibold mb-1">Serie Boleta</label><input type="text" name="config[serie_boleta]" value="{{ $configs['serie_boleta'] ?? 'B001' }}" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg"></div>
                    <div><label class="block text-sm font-semibold mb-1">Serie Factura</label><input type="text" name="config[serie_factura]" value="{{ $configs['serie_factura'] ?? 'F001' }}" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg"></div>
                </div>
                <button class="gradient-primary text-white px-6 py-3 rounded-lg font-semibold"><i class="fas fa-save mr-2"></i>Guardar</button>
            </form>
        </div>
        ...
        <!-- Ticket -->
        <div x-show="tab=='ticket'" x-cloak class="bg-white rounded-2xl shadow-md p-6">
            ...
            <form method="POST" action="{{ route('configuracion.general') }}" class="space-y-4">
                ...
                <div><label class="block text-sm font-semibold mb-1">Ancho papel (mm)</label>
                    <select name="config[ancho_ticket]" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg">
