...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
    ...
    <div class="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl shadow-md p-6 text-white">
        ...
        <form method="POST" action="{{ route('backup.crear') }}">
            @csrf
            <input type="text" name="nombre" placeholder="Nombre (opcional)" class="w-full px-3 py-2 mb-2 rounded-lg text-slate-200">
            <textarea name="observaciones" placeholder="Observaciones..." rows="2" class="w-full px-3 py-2 mb-3 rounded-lg text-slate-200"></textarea>
            <button class="w-full bg-slate-900 border border-slate-800 text-emerald-700 py-2.5 rounded-lg font-semibold hover:bg-emerald-50">
    ...
    <div class="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl shadow-md p-6 text-white">
        ...
        <form method="POST" action="{{ route('backup.restaurar-archivo') }}" enctype="multipart/form-data" onsubmit="return confirm('⚠️ ATENCIÓN: Esta acción reemplazará TODOS los datos actuales. ¿Continuar?')">
            @csrf
            <input type="file" name="archivo" accept=".sql" required class="block w-full text-sm mb-3 file:mr-3 file:py-2 file:px-3 file:rounded file:border-0 file:bg-slate-900 border border-slate-800 file:text-blue-700">
            <button class="w-full bg-slate-900 border border-slate-800 text-blue-700 py-2.5 rounded-lg font-semibold hover:bg-blue-50">
    ...
    <div class="bg-gradient-to-br from-red-500 to-rose-600 rounded-2xl shadow-md p-6 text-white">
        ...
        <button onclick="document.getElementById('modal-reset').classList.remove('hidden')" class="w-full bg-slate-900 border border-slate-800 text-red-700 py-2.5 rounded-lg font-semibold hover:bg-red-50">
...
<div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-md overflow-hidden border border-slate-800">
        ...
        @forelse($backups as $b)
            <div class="p-3.5 hover:bg-slate-800 transition">
                <div class="flex items-center justify-between mb-1">
                    <span class="font-bold text-slate-100 text-xs sm:text-sm">{{ $b->nombre }}</span>
                ...
                <div class="flex items-center justify-between text-[11px] text-slate-400 mb-2">
                    ...
                    <span class="text-slate-300">{{ $b->user?->name ?? 'Sistema' }}</span>
                </div>
        ...
        <table class="w-full text-left text-sm border-collapse">
            ...
            @forelse($backups as $b)
                <tr class="hover:bg-slate-800/80 transition">
                    <td class="py-3 px-4 font-bold text-slate-100">{{ $b->nombre }}</td>
                    <td class="py-3 px-4 text-xs font-mono text-slate-400">{{ $b->archivo }}</td>
                    ...
                    <td class="py-3 px-4 text-sm text-slate-300">{{ $b->user?->name ?? '—' }}</td>
    ...
    <div class="p-3 sm:p-4 border-t border-slate-800">{{ $backups->links() }}</div>
</div>
...
<div id="modal-reset" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md p-6">
        <div class="text-center mb-4">
            ...
            <p class="text-sm text-slate-300 mt-2">Esta acción eliminará todos los datos transaccionales (ventas, compras, productos, clientes, etc.). Se conservarán los usuarios y la configuración. Se generará un backup automático antes de proceder.</p>
        </div>
        <form method="POST" action="{{ route('backup.resetear') }}" class="space-y-3">
            ...
            <div>
                ...
                <input type="password" name="password" required class="w-full px-3 py-2.5 border border-slate-600 rounded-lg">
            </div>
            <div class="flex gap-2">
                <button type="button" onclick="document.getElementById('modal-reset').classList.add('hidden')" class="flex-1 py-2.5 bg-slate-700 rounded-lg">Cancelar</button>
