...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    <div class="lg:col-span-1 space-y-5">
        <div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-md p-6">
            <div class="aspect-square bg-slate-900 rounded-xl overflow-hidden mb-4 flex items-center justify-center">
                @if($producto->imagen)
            ...
            <h2 class="text-xl font-bold text-slate-100">{{ $producto->nombre }}</h2>
            <p class="text-sm text-slate-400 font-mono mt-1">{{ $producto->codigo }}</p>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-md p-6">
            <h3 class="font-bold text-slate-100 mb-4"><i class="fas fa-sliders-h mr-2 text-emerald-500"></i>Ajustar Stock</h3>
            <form method="POST" action="{{ route('productos.ajuste-stock', $producto->id) }}" class="space-y-3">
                @csrf
                <select name="tipo" required class="w-full px-3 py-2 border border-slate-600 rounded-lg">
                ...
                <input type="number" step="0.001" name="cantidad" placeholder="Cantidad" required class="w-full px-3 py-2 border border-slate-600 rounded-lg">
                <input type="text" name="motivo" placeholder="Motivo" required class="w-full px-3 py-2 border border-slate-600 rounded-lg">
                <textarea name="observaciones" placeholder="Observaciones" rows="2" class="w-full px-3 py-2 border border-slate-600 rounded-lg"></textarea>
    ...
    <div class="lg:col-span-2 space-y-5">
        <div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-md p-6">
            <h3 class="font-bold text-slate-100 mb-4">Información detallada</h3>
            <div class="grid grid-cols-2 gap-4 text-sm">
                <div><p class="text-slate-400">Precio compra</p><p class="font-semibold">{{ $moneda }}{{ number_format($producto->precio_compra, 2) }}</p></div>
                <div><p class="text-slate-400">Margen</p><p class="font-semibold text-emerald-600">{{ $producto->margen }}%</p></div>
                <div><p class="text-slate-400">Stock mínimo</p><p class="font-semibold">{{ number_format($producto->stock_minimo, 2) }}</p></div>
                <div><p class="text-slate-400">Stock máximo</p><p class="font-semibold">{{ number_format($producto->stock_maximo, 2) }}</p></div>
                <div><p class="text-slate-400">Unidad</p><p class="font-semibold">{{ $producto->unidad_medida }}</p></div>
                <div><p class="text-slate-400">Ubicación</p><p class="font-semibold">{{ $producto->ubicacion ?: '—' }}</p></div>
                <div><p class="text-slate-400">Lote</p><p class="font-semibold">{{ $producto->lote ?: '—' }}</p></div>
                <div><p class="text-slate-400">Vencimiento</p><p class="font-semibold">{{ $producto->fecha_vencimiento?->format('d/m/Y') ?: '—' }}</p></div>
                <div><p class="text-slate-400">Proveedor</p><p class="font-semibold">{{ $producto->proveedor?->razon_social ?: '—' }}</p></div>
                <div><p class="text-slate-400">Código de barras</p><p class="font-semibold font-mono">{{ $producto->codigo_barras ?: '—' }}</p></div>
            </div>
            @if($producto->descripcion)
                <div class="mt-4 pt-4 border-t border-slate-800">
                    <p class="text-slate-400 text-sm mb-1">Descripción</p>
                    <p class="text-slate-200">{{ $producto->descripcion }}</p>
                </div>
            @endif
        </div>

        <div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-md p-6">
            <h3 class="font-bold text-slate-100 mb-4"><i class="fas fa-history mr-2 text-blue-500"></i>Movimientos de Inventario</h3>
