...
@if($turnoActivo)
    <!-- Turno Activo -->
    <div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-md p-4 sm:p-6 mb-5 border border-slate-800">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-5">
            <div>
                ...
                <h2 class="text-xl sm:text-2xl font-black text-slate-100 mt-1.5">{{ $turnoActivo->caja->nombre }}</h2>
                <p class="text-xs sm:text-sm text-slate-400">Abierto el {{ $turnoActivo->fecha_apertura->format('d/m/Y H:i') }} • Cajero: <strong class="text-slate-200">{{ $turnoActivo->user->name }}</strong></p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 pt-4 border-t border-slate-800">
            ...
            <div class="bg-slate-800/80 border border-slate-700 rounded-2xl p-4">
                <h3 class="font-extrabold text-sm sm:text-base text-slate-100 mb-3 flex items-center gap-2">
                ...
                <form method="POST" action="{{ route('caja.movimiento', $turnoActivo->id) }}" class="space-y-3">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        <select name="tipo" class="w-full px-3 py-2 bg-slate-900 border border-slate-800 border border-slate-600 rounded-xl text-xs sm:text-sm font-semibold focus:outline-none focus:border-amber-500">
                            <option value="ingreso">🟢 Ingreso (+)</option>
                            <option value="egreso">🔴 Egreso (-)</option>
                        </select>
                        <input type="number" step="0.01" name="monto" placeholder="Monto (0.00)" required 
                               class="w-full px-3 py-2 bg-slate-900 border border-slate-800 border border-slate-600 rounded-xl text-xs sm:text-sm font-black focus:outline-none focus:border-amber-500">
                    </div>
                    <input type="text" name="concepto" placeholder="Concepto (ej: Pago a proveedor de pan)" required 
                           class="w-full px-3 py-2 bg-slate-900 border border-slate-800 border border-slate-600 rounded-xl text-xs sm:text-sm font-medium focus:outline-none focus:border-amber-500">
            ...
            <div class="bg-slate-800/80 border border-slate-700 rounded-2xl p-4 flex flex-col">
                <h3 class="font-extrabold text-sm sm:text-base text-slate-100 mb-3 flex items-center gap-2">
                ...
                <div class="max-h-56 overflow-y-auto space-y-2 flex-1 pr-1">
                    @forelse($turnoActivo->movimientos as $m)
                        <div class="flex justify-between items-center bg-slate-900 border border-slate-800 border border-slate-700 rounded-xl p-2.5 shadow-xs">
    ...
    <div id="modal-cerrar" class="fixed inset-0 bg-slate-950/70 backdrop-blur-xs z-50 hidden flex items-center justify-center p-3 sm:p-4">
        <div class="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-md p-5 sm:p-6 shadow-2xl border border-slate-800 transform transition-all">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    <h3 class="text-lg font-black text-slate-100 leading-tight">Cerrar Turno de Caja</h3>
            ...
            <form method="POST" action="{{ route('caja.cerrar', $turnoActivo->id) }}" class="space-y-4">
                @csrf
                <div>
                    <label class="block text-xs font-bold uppercase text-slate-300 mb-1.5">Monto físico contado en caja *</label>
                    <input type="number" step="0.01" name="monto_cierre" required 
                           class="w-full px-3 py-2.5 border border-slate-600 rounded-2xl text-2xl font-black text-center text-slate-100 bg-slate-800 focus:bg-slate-900 border border-slate-800 focus:outline-none focus:border-amber-500"
                           placeholder="0.00">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase text-slate-300 mb-1.5">Observaciones</label>
                    <textarea name="observaciones" placeholder="Detalle de cierre o diferencias..." rows="2" 
                              class="w-full px-3 py-2 border border-slate-600 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-amber-500"></textarea>
                </div>
                <div class="flex gap-2 pt-2">
                    <button type="button" onclick="document.getElementById('modal-cerrar').classList.add('hidden')" 
                            class="w-1/3 py-3 bg-slate-900 hover:bg-slate-700 text-slate-200 rounded-xl font-bold text-xs transition">
                        Cancelar
                    </button>
                    <button type="submit" 
...
@else
    <!-- Sin turno activo -->
    <div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-md p-6 sm:p-10 mb-5 text-center border border-slate-800">
        ...
        <h2 class="text-lg sm:text-2xl font-black text-slate-100 mb-1">No tienes un turno de caja abierto</h2>
        <p class="text-xs sm:text-sm text-slate-400 mb-6 max-w-md mx-auto">Para realizar ventas y emitir comprobantes, debes iniciar y abrir un turno de caja con tu saldo inicial.</p>
    ...
    <div id="modal-abrir" class="fixed inset-0 bg-slate-950/70 backdrop-blur-xs z-50 hidden flex items-center justify-center p-3 sm:p-4">
        <div class="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-md p-5 sm:p-6 shadow-2xl border border-slate-800 transform transition-all mx-2">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    <h3 class="text-lg font-black text-slate-100 leading-tight">Abrir Turno de Caja</h3>
            ...
            @if($cajas->count() == 0)
                ...
                <form method="POST" action="{{ route('caja.store') }}" class="space-y-3">
                    ...
                    <input type="text" name="nombre" placeholder="Nombre de la caja (ej: Caja Principal)" required 
                           class="w-full px-3 py-2.5 border border-slate-600 rounded-xl text-xs sm:text-sm font-semibold">
            ...
            @else
                <form method="POST" action="{{ route('caja.abrir') }}" class="space-y-3.5">
                    @csrf
                    <div>
                        <label class="block text-xs font-bold uppercase text-slate-300 mb-1">Caja</label>
                        <select name="caja_id" required class="w-full px-3 py-2.5 bg-slate-800 border border-slate-600 rounded-xl text-xs sm:text-sm font-bold text-slate-100 focus:bg-slate-900 border border-slate-800 focus:outline-none focus:border-amber-500">
                            @foreach($cajas as $c)<option value="{{ $c->id }}">{{ $c->nombre }}</option>@endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-xs font-bold uppercase text-slate-300 mb-1">Monto de Apertura (Saldo Inicial) *</label>
                        <div class="relative">
                    ...
                    <div>
                        <label class="block text-xs font-bold uppercase text-slate-300 mb-1">Observaciones</label>
...
<div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-md overflow-hidden border border-slate-800">
    <div class="p-4 sm:p-5 border-b border-slate-800 flex items-center justify-between">
        <h3 class="font-extrabold text-slate-100 text-sm sm:text-base flex items-center gap-2">
            ...
            <div class="p-3.5 hover:bg-slate-800 transition">
                <div class="flex items-center justify-between mb-1">
                    <div class="flex items-center gap-2">
                        <span class="font-bold text-slate-100 text-xs">{{ $t->caja->nombre }}</span>
                        @if($t->estado == 'abierto')
                            <span class="bg-green-100 text-green-700 px-2 py-0.2 rounded-full text-[10px] font-bold">Abierto</span>
                        @else
                            <span class="bg-slate-900 text-slate-300 px-2 py-0.2 rounded-full text-[10px] font-bold">Cerrado</span>
                        @endif
                    </div>
                ...
                @if($t->estado != 'abierto')
                    <div class="pt-2 border-t border-slate-800 flex gap-1.5 justify-end">
                        ...
                        <a href="{{ route('caja.cierre', $t->id) }}" class="px-3 py-1.5 bg-slate-900 hover:bg-slate-700 text-slate-200 rounded-xl text-xs font-bold transition flex items-center gap-1">
        ...
        <table class="w-full text-left text-sm border-collapse">
            ...
            @forelse($turnos as $t)
                <tr class="hover:bg-slate-800/80 transition">
                    <td class="py-3 px-4 font-bold text-slate-100">{{ $t->caja->nombre }}</td>
                    <td class="py-3 px-4 text-slate-300">{{ $t->user->name }}</td>
                    ...
                    <td class="py-3 px-4 text-center">
                        ...
                        @else
                            <span class="bg-slate-900 text-slate-200 px-2.5 py-1 rounded-full text-xs font-bold">Cerrado</span>
                        @endif
                    </td>
