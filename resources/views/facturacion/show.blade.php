...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    ...
    <div class="lg:col-span-2 space-y-5">
        <div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-md p-6">
            <div class="flex flex-wrap justify-between items-start gap-3 pb-4 border-b border-slate-700">
                <div>
                    <p class="text-xs text-slate-400 uppercase">{{ $comprobante->tipo_documento_nombre }}</p>
                    <h2 class="text-2xl font-bold text-slate-100 font-mono">{{ $comprobante->numero_completo }}</h2>
                    <p class="text-sm text-slate-400 mt-1">Emitida el {{ $comprobante->fecha_emision->format('d/m/Y') }} a las {{ $comprobante->hora_emision }}</p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mt-4">
                <div>
                    <h4 class="text-xs uppercase text-slate-400 font-semibold mb-2"><i class="fas fa-store mr-1"></i>Emisor</h4>
                    <p class="font-bold">{{ $comprobante->emisor_razon_social }}</p>
                    <p class="text-sm text-slate-300">RUC: {{ $comprobante->emisor_ruc }}</p>
                </div>
                <div>
                    <h4 class="text-xs uppercase text-slate-400 font-semibold mb-2"><i class="fas fa-user mr-1"></i>Receptor</h4>
                    <p class="font-bold">{{ $comprobante->receptor_razon_social }}</p>
                    <p class="text-sm text-slate-300">{{ $comprobante->receptor_tipo_doc_label }}: {{ $comprobante->receptor_numero_doc }}</p>
                    @if($comprobante->receptor_direccion)
                        <p class="text-xs text-slate-400">{{ $comprobante->receptor_direccion }}</p>
                    @endif
                </div>
            </div>
            ...
            <!-- Totales -->
            <div class="mt-4 pt-4 border-t border-slate-700 space-y-2 text-right">
                <div class="flex justify-end gap-8 text-sm"><span class="text-slate-300">Op. Gravadas:</span><span class="w-32 font-semibold">{{ $moneda }}{{ number_format($comprobante->total_gravadas, 2) }}</span></div>
                <div class="flex justify-end gap-8 text-sm"><span class="text-slate-300">IGV (18%):</span><span class="w-32 font-semibold">{{ $moneda }}{{ number_format($comprobante->total_igv, 2) }}</span></div>
                @if($comprobante->total_descuentos > 0)
                    <div class="flex justify-end gap-8 text-sm"><span class="text-slate-300">Descuentos:</span><span class="w-32 font-semibold text-red-600">-{{ $moneda }}{{ number_format($comprobante->total_descuentos, 2) }}</span></div>
                @endif
                ...
                <p class="text-xs text-slate-400 italic mt-1">{{ $comprobante->importe_letras }}</p>
            </div>
        </div>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-md p-6">
            ...
            <div class="bg-{{ $color }}-50 border-l-4 border-{{ $color }}-500 p-4 rounded">
                ...
                @if($comprobante->fecha_envio_sunat)
                    <p class="text-xs text-slate-400 mt-2">Enviado el {{ $comprobante->fecha_envio_sunat->format('d/m/Y H:i:s') }} • Intentos: {{ $comprobante->intentos_envio }}</p>
                @endif
            </div>
        </div>
        @endif
    ...
    <div class="space-y-5">
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-md p-6 text-center">
            <h4 class="font-bold mb-3"><i class="fas fa-qrcode mr-1 text-slate-200"></i>Código QR SUNAT</h4>
            ...
            <p class="text-xs text-slate-400 mt-2">Escanea para validar en SUNAT</p>
        </div>
        @endif
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-md p-6">
            ...
            <dl class="text-sm space-y-2">
                <div class="flex justify-between"><dt class="text-slate-400">Tipo:</dt><dd class="font-semibold">{{ $comprobante->tipo_documento }} - {{ $comprobante->tipo_documento_nombre }}</dd></div>
                <div class="flex justify-between"><dt class="text-slate-400">Serie:</dt><dd class="font-mono font-semibold">{{ $comprobante->serie }}</dd></div>
                <div class="flex justify-between"><dt class="text-slate-400">Correlativo:</dt><dd class="font-mono font-semibold">{{ $comprobante->numero }}</dd></div>
                <div class="flex justify-between"><dt class="text-slate-400">Moneda:</dt><dd class="font-semibold">{{ $comprobante->moneda }}</dd></div>
                @if($comprobante->hash)
                    <div><dt class="text-slate-400 text-xs">Hash XML:</dt><dd class="font-mono text-[10px] break-all bg-slate-800 p-2 rounded mt-1">{{ Str::limit($comprobante->hash, 80) }}</dd></div>
                @endif
            </dl>
        </div>

        <a href="{{ route('facturacion.index') }}" class="block text-center bg-slate-900 hover:bg-slate-700 py-2.5 rounded-lg text-slate-200 font-semibold">
...
<div id="modal-anular" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl w-full max-w-md p-6">
        ...
        <p class="text-sm text-slate-300 mb-4">Elige el método de anulación según SUNAT:</p>
        <form method="POST" action="{{ route('facturacion.anular', $comprobante->id) }}" class="space-y-3">
            ...
            <div>
                ...
                <select name="metodo" class="w-full px-3 py-2 border border-slate-600 rounded-lg">
            ...
            <div>
                ...
                <textarea name="motivo" rows="3" required class="w-full px-3 py-2 border border-slate-600 rounded-lg" placeholder="Ej: Anulación por error en datos del cliente"></textarea>
            </div>
            <div class="flex gap-2 pt-2">
                <button type="button" onclick="document.getElementById('modal-anular').classList.add('hidden')" class="flex-1 py-2.5 bg-slate-700 rounded-lg">Cancelar</button>
