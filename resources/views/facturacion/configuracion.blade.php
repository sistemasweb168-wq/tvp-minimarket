...
<div x-data="{ tab: 'sunat' }" class="grid grid-cols-1 lg:grid-cols-4 gap-5">
    ...
    <div class="lg:col-span-3">
        <!-- Datos SUNAT -->
        <div x-show="tab=='sunat'" class="bg-white rounded-2xl shadow-md p-6">
            ...
            <form method="POST" action="{{ route('facturacion.configuracion.guardar') }}" enctype="multipart/form-data">
                ...
                <h3 class="font-bold text-slate-700 mb-3">Credenciales SOL (Producción)</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
                    <div>
                        ...
                        <input type="text" name="sunat_usuario_sol" value="{{ $empresa->sunat_usuario_sol }}"
                               class="w-full px-3 py-2.5 border border-slate-300 rounded-lg" placeholder="MIRUC + MODDATOS">
                    </div>
                    <div>
                        ...
                        <input type="password" name="sunat_clave_sol" value="{{ $empresa->sunat_clave_sol }}"
                               class="w-full px-3 py-2.5 border border-slate-300 rounded-lg" placeholder="••••••••">
                    </div>
                </div>

                <h3 class="font-bold text-slate-700 mb-3">Ubicación (Ubigeo SUNAT)</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5" x-data="ubigeoSearch()">
                    <div class="md:col-span-2 relative">
                        ...
                        <div x-show="open && resultados.length > 0" x-cloak class="absolute z-10 w-full mt-1 bg-white border border-slate-200 rounded-lg shadow-lg max-h-64 overflow-y-auto">
                            <template x-for="r in resultados" :key="r.codigo">
                                <button type="button" @click="seleccionar(r)" class="w-full text-left px-3 py-2 hover:bg-emerald-50 border-b border-slate-100 text-sm">
                                    <span class="font-mono text-xs text-slate-500" x-text="r.codigo"></span>
                    ...
                    <div>
                        ...
                        <input type="text" name="ubigeo" x-ref="ubigeoInput" value="{{ $empresa->ubigeo ?? '150101' }}" maxlength="6"
                               class="w-full px-3 py-2.5 border border-slate-300 rounded-lg font-mono font-bold" placeholder="150101">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold mb-1">Departamento</label>
                        <input type="text" name="departamento" x-ref="dptoInput" value="{{ $empresa->departamento ?? 'LIMA' }}" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold mb-1">Provincia</label>
                        <input type="text" name="provincia" x-ref="provInput" value="{{ $empresa->provincia ?? 'LIMA' }}" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold mb-1">Distrito</label>
                        <input type="text" name="distrito" x-ref="distInput" value="{{ $empresa->distrito ?? 'LIMA' }}" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg">
                    </div>
                </div>
        ...
        <!-- Certificado -->
        <div x-show="tab=='certificado'" x-cloak class="bg-white rounded-2xl shadow-md p-6">
            ...
            <form method="POST" action="{{ route('facturacion.configuracion.guardar') }}" enctype="multipart/form-data">
                ...
                <div class="mb-5">
                    ...
                    <input type="password" name="sunat_certificado_password" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg" placeholder="••••••••">
                    <p class="text-xs text-slate-500 mt-1">Solo necesario si el certificado es .pfx</p>
                </div>
        ...
        <!-- Series -->
        <div x-show="tab=='series'" x-cloak class="bg-white rounded-2xl shadow-md p-6">
            <h2 class="text-xl font-bold mb-5"><i class="fas fa-hashtag mr-2 text-emerald-500"></i>Series y Correlativos</h2>

            <form method="POST" action="{{ route('facturacion.series.crear') }}" class="grid grid-cols-1 md:grid-cols-4 gap-3 mb-5 pb-5 border-b border-slate-200">
                @csrf
                <select name="tipo_documento" required class="px-3 py-2 border border-slate-300 rounded-lg">
                    <option value="01">01 - Factura</option>
                    <option value="03">03 - Boleta</option>
                    <option value="07">07 - Nota Crédito</option>
                    <option value="08">08 - Nota Débito</option>
                </select>
                <input name="serie" required maxlength="4" placeholder="Ej: F001" class="px-3 py-2 border border-slate-300 rounded-lg font-mono">
                <input name="descripcion" placeholder="Descripción" class="px-3 py-2 border border-slate-300 rounded-lg">
                ...
                @forelse($series as $s)
                    <tr class="border-b">
                        ...
                        <td class="py-2 text-center">
                            ...
                            @else
                                <span class="bg-slate-100 text-slate-500 px-2 py-1 rounded-full text-xs">Inactiva</span>
                            @endif
                        </td>
                    </tr>
                @empty
        ...
        <!-- Ayuda -->
        <div x-show="tab=='ayuda'" x-cloak class="bg-white rounded-2xl shadow-md p-6">
            ...
            <div class="space-y-4">
                <div class="border-l-4 border-emerald-500 bg-emerald-50 p-4 rounded">
                    ...
                    <p class="text-sm text-emerald-700">El sistema usa automáticamente las credenciales de prueba <code class="bg-white px-2 py-1 rounded">MODDATOS</code> con tu RUC. Útil para validar el flujo sin afectar producción.</p>
                </div>
