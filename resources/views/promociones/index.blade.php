...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    @forelse($promociones as $p)
        <div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-md p-5 relative overflow-hidden hover:shadow-lg transition">
            @if($p->vigente)
                <span class="absolute top-3 right-3 bg-green-100 text-green-700 px-2 py-1 rounded-full text-xs font-semibold">VIGENTE</span>
            @else
                <span class="absolute top-3 right-3 bg-slate-900 text-slate-400 px-2 py-1 rounded-full text-xs">Programada/Vencida</span>
            @endif
            ...
            <div class="space-y-1 text-sm">
                ...
                <p class="text-xs text-slate-400"><i class="far fa-calendar"></i> {{ $p->fecha_inicio->format('d/m/Y') }} - {{ $p->fecha_fin->format('d/m/Y') }}</p>
            </div>
...
<div x-show="open" x-cloak class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" style="display:none;">
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto" @click.outside="open=false">
        ...
        <form :action="edit ? `/promociones/${edit.id}` : '{{ route('promociones.store') }}'" method="POST" class="space-y-3">
            ...
            <div><label class="text-sm font-semibold">Nombre</label><input name="nombre" :value="edit?.nombre || ''" required class="w-full px-3 py-2 border border-slate-600 rounded-lg"></div>
            <div><label class="text-sm font-semibold">Descripción</label><textarea name="descripcion" rows="2" class="w-full px-3 py-2 border border-slate-600 rounded-lg" x-text="edit?.descripcion || ''"></textarea></div>
            <div class="grid grid-cols-2 gap-3">
                <div><label class="text-sm font-semibold">Tipo</label>
                    <select name="tipo" class="w-full px-3 py-2 border border-slate-600 rounded-lg">
                ...
                <div><label class="text-sm font-semibold">Valor</label><input type="number" step="0.01" name="valor" :value="edit?.valor || 0" required class="w-full px-3 py-2 border border-slate-600 rounded-lg"></div>
            </div>
            <div class="grid grid-cols-2 gap-3">
                <div><label class="text-sm font-semibold">Producto (opcional)</label>
                    <select name="producto_id" class="w-full px-3 py-2 border border-slate-600 rounded-lg">
                ...
                <div><label class="text-sm font-semibold">Categoría (opcional)</label>
                    <select name="categoria_id" class="w-full px-3 py-2 border border-slate-600 rounded-lg">
                        <option value="">— Cualquiera —</option>
            ...
            <div class="grid grid-cols-3 gap-3">
                <div><label class="text-sm font-semibold">Desde</label><input type="date" name="fecha_inicio" :value="edit?.fecha_inicio || '{{ now()->toDateString() }}'" required class="w-full px-3 py-2 border border-slate-600 rounded-lg"></div>
                <div><label class="text-sm font-semibold">Hasta</label><input type="date" name="fecha_fin" :value="edit?.fecha_fin || '{{ now()->addDays(30)->toDateString() }}'" required class="w-full px-3 py-2 border border-slate-600 rounded-lg"></div>
                <div><label class="text-sm font-semibold">Cant. mín.</label><input type="number" name="cantidad_minima" :value="edit?.cantidad_minima || 1" class="w-full px-3 py-2 border border-slate-600 rounded-lg"></div>
            </div>
