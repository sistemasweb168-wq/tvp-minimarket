...
<div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-md p-4 sm:p-5 mb-4 border border-slate-800">
    <form method="GET" class="flex flex-col sm:flex-row gap-3 items-stretch sm:items-end">
        <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div><label class="block text-xs font-bold uppercase text-slate-300 mb-1">Desde</label><input type="date" name="desde" value="{{ $desde }}" class="w-full px-3 py-2 border border-slate-600 rounded-xl text-xs sm:text-sm focus:outline-none focus:border-amber-500"></div>
            <div><label class="block text-xs font-bold uppercase text-slate-300 mb-1">Hasta</label><input type="date" name="hasta" value="{{ $hasta }}" class="w-full px-3 py-2 border border-slate-600 rounded-xl text-xs sm:text-sm focus:outline-none focus:border-amber-500"></div>
        </div>
...
<div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-md overflow-hidden border border-slate-800">
    <div class="p-4 border-b border-slate-800">
        <h3 class="font-extrabold text-xs sm:text-sm text-slate-100">
        ...
        @forelse($productos as $i => $p)
            <div class="p-3.5 hover:bg-slate-800 transition flex items-center gap-3">
                <span class="w-8 h-8 rounded-xl inline-flex items-center justify-center text-white text-xs font-black flex-shrink-0 shadow-xs
                    {{ $i==0?'bg-amber-500':($i==1?'bg-slate-400':($i==2?'bg-amber-700':'bg-slate-300 text-slate-200')) }}">
                    #{{ $i+1 }}
                </span>
                
                <div class="flex-1 min-w-0">
                    <h4 class="font-bold text-slate-100 text-xs sm:text-sm truncate">{{ $p->nombre }}</h4>
                    <p class="text-[11px] text-slate-400 font-mono">{{ $p->codigo }} • {{ $p->categoria ?: 'Sin categoría' }}</p>
                    <div class="flex items-center justify-between mt-1 pt-1 border-t border-slate-800 text-xs">
                        <span class="text-slate-300 font-medium">{{ number_format($p->cantidad_vendida, 2) }} unid.</span>
                ...
                <tr class="hover:bg-slate-800/80 transition">
                    <td class="py-3 px-4">
                        <span class="w-7 h-7 rounded-lg inline-flex items-center justify-center text-white text-xs font-black
                            {{ $i==0?'bg-amber-500':($i==1?'bg-slate-400':($i==2?'bg-amber-700':'bg-slate-700 text-slate-200')) }}">{{ $i+1 }}</span>
                    </td>
                    <td class="py-3 px-4 font-mono text-xs">{{ $p->codigo }}</td>
                    <td class="py-3 px-4 font-bold text-slate-100">{{ $p->nombre }}</td>
                    <td class="py-3 px-4 text-xs text-slate-400">{{ $p->categoria ?: '—' }}</td>
                    <td class="py-3 px-4 text-right font-semibold">{{ number_format($p->cantidad_vendida, 2) }}</td>
                    <td class="py-3 px-4 text-right text-slate-300">{{ $moneda }}{{ number_format($p->precio_promedio, 2) }}</td>
