...
@section('content')
<div class="bg-white rounded-2xl shadow-md p-4 sm:p-5 mb-4 flex justify-between items-center border border-slate-100">
    <div>
        ...
        @forelse($productos as $p)
            ...
            <div class="p-3.5 hover:bg-slate-50 transition">
                ...
                <div class="flex items-center justify-between pt-1.5 border-t border-slate-100 text-xs">
                    <span class="text-slate-600 font-semibold">Stock: {{ number_format($p->stock, 2) }} {{ $p->unidad_medida }}</span>
                    <span class="text-slate-500 font-medium">Vence: <strong class="text-slate-700">{{ $p->fecha_vencimiento->format('d/m/Y') }}</strong></span>
                </div>
            </div>
        @empty
        ...
        <table class="w-full text-left text-sm border-collapse">
            ...
            @forelse($productos as $p)
                ...
                <tr class="hover:bg-slate-50/80 transition">
                    ...
                    <td class="py-3 px-4 text-sm text-slate-600">{{ $p->categoria?->nombre ?? '—' }}</td>
                    <td class="py-3 px-4 font-mono text-xs text-slate-500">{{ $p->lote ?: '—' }}</td>
                    <td class="py-3 px-4 text-right font-black text-slate-800">{{ number_format($p->stock, 2) }}</td>
                    <td class="py-3 px-4 text-sm font-semibold text-slate-700">{{ $p->fecha_vencimiento->format('d/m/Y') }}</td>
