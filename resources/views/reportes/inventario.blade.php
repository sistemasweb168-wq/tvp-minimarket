...
<div class="bg-white rounded-2xl shadow-md p-4 sm:p-5 mb-4 flex justify-between items-center">
    <h3 class="font-extrabold text-sm sm:text-base text-slate-800"><i class="fas fa-boxes text-emerald-500 mr-2"></i>Detalle de Inventario</h3>
            ...
            <div class="p-3.5 hover:bg-slate-50 transition">
                <div class="flex items-center justify-between mb-1">
                    <div class="flex items-center gap-1.5 min-w-0 pr-2">
                        <span class="font-bold text-slate-800 text-xs sm:text-sm truncate">{{ $p->nombre }}</span>
                    </div>
                    @if($p->stock_bajo)
                        <span class="bg-red-100 text-red-700 px-2 py-0.2 rounded-full text-[10px] font-bold flex-shrink-0">Bajo</span>
                    @elseif($p->stock == 0)
                        <span class="bg-slate-100 text-slate-700 px-2 py-0.2 rounded-full text-[10px] font-bold flex-shrink-0">Agotado</span>
                    @else
                ...
                <div class="flex items-center gap-2 text-[11px] text-slate-400 font-mono mb-1.5">
                    <span>{{ $p->codigo }}</span>
                    <span>•</span>
                    <span class="text-slate-600 font-sans">{{ $p->categoria?->nombre ?? 'Sin categoría' }}</span>
                </div>

                <div class="grid grid-cols-2 gap-2 pt-1.5 border-t border-slate-100 text-xs">
                    <div>
                        <span class="text-[10px] uppercase font-bold text-slate-400 block">Stock Actual / Mín</span>
                        <span class="font-black {{ $p->stock_bajo ? 'text-red-600' : 'text-slate-800' }}">{{ number_format($p->stock, 2) }}</span>
        ...
        <table class="w-full text-left text-sm border-collapse">
            ...
            @foreach($productos as $p)
                <tr class="hover:bg-slate-50/80 transition">
                    ...
                    <td class="py-3 px-4 text-xs text-slate-600">{{ $p->categoria?->nombre ?? '—' }}</td>
                    <td class="py-3 px-4 text-right font-black {{ $p->stock_bajo ? 'text-red-600' : 'text-slate-800' }}">{{ number_format($p->stock, 2) }}</td>
                    <td class="py-3 px-4 text-right text-xs text-slate-400">{{ number_format($p->stock_minimo, 2) }}</td>
                    <td class="py-3 px-4 text-right text-slate-600">{{ $moneda }}{{ number_format($p->precio_compra, 2) }}</td>
                    <td class="py-3 px-4 text-right font-bold text-emerald-600">{{ $moneda }}{{ number_format($p->precio_venta, 2) }}</td>
                    <td class="py-3 px-4 text-right font-black text-slate-800">{{ $moneda }}{{ number_format($p->stock * $p->precio_compra, 2) }}</td>
                    <td class="py-3 px-4 text-center">
                        @if($p->stock_bajo)
                            <span class="bg-red-100 text-red-700 px-2.5 py-1 rounded-full text-xs font-bold">Bajo</span>
                        @elseif($p->stock == 0)
                            <span class="bg-slate-100 text-slate-700 px-2.5 py-1 rounded-full text-xs font-bold">Agotado</span>
                        @else
