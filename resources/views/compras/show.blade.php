...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    <div class="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl shadow-md p-6">
        <div class="flex justify-between mb-6">
            <div>
                ...
                <p class="text-slate-400">{{ $compra->fecha_compra->format('d/m/Y H:i') }}</p>
        ...
        <table class="w-full">
            <thead class="border-b-2 text-xs uppercase text-slate-400">
            ...
            @foreach($compra->detalles as $d)
                <tr class="border-b">
                    <td class="py-3"><p class="font-semibold">{{ $d->descripcion }}</p><p class="text-xs text-slate-400">{{ $d->codigo }}</p></td>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-md p-6 h-fit">
        <h3 class="font-bold mb-3">Información</h3>
        <div class="space-y-2 text-sm">
            <p><span class="text-slate-400">Proveedor:</span> <strong>{{ $compra->proveedor->razon_social }}</strong></p>
            <p><span class="text-slate-400">Registrado por:</span> <strong>{{ $compra->user->name }}</strong></p>
            <p><span class="text-slate-400">Forma de pago:</span> <strong>{{ ucfirst($compra->forma_pago) }}</strong></p>
        </div>
        @if($compra->observaciones)
            <p class="mt-4 text-sm text-slate-300 italic">{{ $compra->observaciones }}</p>
        @endif
        <a href="{{ route('compras.index') }}" class="block text-center mt-4 bg-slate-900 hover:bg-slate-700 py-2.5 rounded-lg">Volver</a>
    </div>
</div>
@endsection
