...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    <div class="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl shadow-md p-6">
        <div class="flex justify-between items-start mb-6">
            <div>
                <h2 class="text-2xl font-bold text-slate-100">Ticket {{ $venta->numero_ticket }}</h2>
                <p class="text-slate-400">{{ $venta->fecha_venta->format('d/m/Y H:i:s') }}</p>
            </div>
        ...
        <div class="mt-6 pt-4 border-t-2 border-slate-700 space-y-2 text-right">
            <div class="flex justify-end gap-8"><span class="text-slate-300">Subtotal:</span><span class="font-semibold w-32">{{ $moneda }}{{ number_format($venta->subtotal, 2) }}</span></div>
            @if($venta->descuento > 0)
                <div class="flex justify-end gap-8"><span class="text-slate-300">Descuento:</span><span class="font-semibold w-32 text-red-600">-{{ $moneda }}{{ number_format($venta->descuento, 2) }}</span></div>
            @endif
            <div class="flex justify-end gap-8"><span class="text-slate-300">Impuesto:</span><span class="font-semibold w-32">{{ $moneda }}{{ number_format($venta->impuesto, 2) }}</span></div>
            <div class="flex justify-end gap-8 text-2xl pt-2 border-t border-slate-700"><span class="font-bold">TOTAL:</span><span class="font-bold w-32 text-emerald-600">{{ $moneda }}{{ number_format($venta->total, 2) }}</span></div>
        </div>
    </div>
    ...
    <div class="space-y-5">
        <div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-md p-6">
            ...
            <div class="space-y-2 text-sm">
                <p><span class="text-slate-400">Cliente:</span> <strong>{{ $venta->cliente?->nombre_completo ?? 'Genérico' }}</strong></p>
                <p><span class="text-slate-400">Cajero:</span> <strong>{{ $venta->user->name }}</strong></p>
                <p><span class="text-slate-400">Forma de pago:</span> <strong>{{ ucfirst($venta->forma_pago) }}</strong></p>
                <p><span class="text-slate-400">Recibido:</span> <strong>{{ $moneda }}{{ number_format($venta->monto_recibido, 2) }}</strong></p>
                <p><span class="text-slate-400">Cambio:</span> <strong>{{ $moneda }}{{ number_format($venta->cambio, 2) }}</strong></p>
            </div>
        </div>

        <div class="bg-slate-900 border border-slate-800 rounded-2xl shadow-md p-6 space-y-2">
            ...
            @if($venta->estado == 'completada' && ($empresaGlobal->facturacion_electronica_activa ?? false))
                ...
                @else
                    <form method="POST" action="{{ route('facturacion.emitir', $venta->id) }}" class="space-y-2">
                        @csrf
                        <p class="text-xs font-semibold text-slate-300 mb-1">Emitir comprobante electrónico:</p>
            ...
            @endif
            <a href="{{ route('ventas.index') }}" class="block text-center bg-slate-900 hover:bg-slate-700 text-slate-200 py-2.5 rounded-lg">Volver</a>
        </div>
    </div>
</div>
@endsection
